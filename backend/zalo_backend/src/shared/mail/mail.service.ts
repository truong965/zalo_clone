...
export class MailService {
  ...
  async sendOtpEmail(email: string, otp: string) {
    ...
    return this.sendMail(email, subject, this.getOtpTemplate(otp, 'đổi mật khẩu'));
  }

  async sendTwoFactorEmail(email: string, otp: string) {
    const subject = 'Zalo Clone - Mã xác thực 2 bước';
    return this.sendMail(email, subject, this.getOtpTemplate(otp, 'xác thực đăng nhập 2 bước'));
  }

  private getOtpTemplate(otp: string, actionName: string): string {
    return `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;">
        <h2 style="color: #0068ff; text-align: center;">Zalo Clone</h2>
        <p>Chào bạn,</p>
        <p>Đây là mã OTP để <b>${actionName}</b> của bạn. Mã này có hiệu lực trong <b>5 phút</b>.</p>
        ...
        <p style="margin-top: 20px;">Nếu bạn không yêu cầu hành động này, vui lòng bỏ qua email này.</p>
