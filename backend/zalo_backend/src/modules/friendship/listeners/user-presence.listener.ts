...
export class UserPresenceListener {
      ...
      async handleUserConnected(payload: { userId: string; socketId: string; connectedAt: Date }) {
      ...
      async handleUserDisconnected(payload: { userId: string; socketId: string; reason: string }) {
      ...
      private async notifyFriendsPresence(
            ...
            try {
                  ...
                  const friendIds = await this.friendshipService.getFriendIdsForPresence(userId);
                  ...
                  const socketEvent: ISocketEmitEvent = {
                        event: (isOnline ? SocketEvents.FRIEND_ONLINE : SocketEvents.FRIEND_OFFLINE) as any,
                        data: eventPayload,
                        userIds: friendIds,
                  };
