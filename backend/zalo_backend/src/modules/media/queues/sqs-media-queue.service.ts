...
export class SqsMediaQueueService implements IMediaQueueService {
  ...
  async enqueueImageProcessing(payload: ImageProcessingJob): Promise<string> {
  ...
  async enqueueVideoProcessing(payload: VideoProcessingJob): Promise<string> {
  ...
  async enqueueFileProcessing(
    ...
    const queueUrl =
      mediaType === MediaType.VIDEO ? this.videoQueueUrl : this.imageQueueUrl;
  ...
  async getQueueStats() {
    ...
    const waiting = (imageAttrs.waiting ?? 0) + (videoAttrs.waiting ?? 0);
    const active = (imageAttrs.active ?? 0) + (videoAttrs.active ?? 0);
    const delayed = (imageAttrs.delayed ?? 0) + (videoAttrs.delayed ?? 0);
  ...
  private async fetchAttributes(
    ...
    try {
      ...
      this.logger.warn(
        `Failed to get SQS attributes for ${queueUrl}: ${(e as Error).message}`,
      );
