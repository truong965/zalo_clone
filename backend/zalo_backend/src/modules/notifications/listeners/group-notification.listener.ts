...
export class GroupNotificationListener {
  ...
  private async processGroupCreated(
    event: ConversationCreatedEvent,
  ): Promise<void> {
  ...
  private async processMemberAdded(
    ...
    const memberLabel =
      addedCount === 1 ? 'thành viên' : `${addedCount} thành viên`;
    ...
    for (const recipientId of remainingIds) {
      pushPromises.push(
        this.safeSendGroupPush({
          ...
          conversationId,
          subtype: isKicked
            ? GROUP_SUBTYPE.MEMBER_REMOVED
            : GROUP_SUBTYPE.MEMBER_LEFT,
          groupName,
  ...
  private async processMemberPromoted(
    event: ConversationMemberPromotedEvent,
  ): Promise<void> {
  ...
  private async processMemberDemoted(
    event: ConversationMemberDemotedEvent,
  ): Promise<void> {
    const { conversationId, memberId } = event;
