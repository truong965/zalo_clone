...
export class ReminderSchedulerService {
  ...
  async pollDueReminders() {
    ...
    try {
      ...
      this.logger.log(
        `Found ${dueReminders.length} due reminder(s) to trigger`,
      );
      ...
      for (const reminder of dueReminders) {
        try {
          ...
          this.logger.log(
            `Triggered reminder ${reminder.id} for user ${reminder.userId}`,
          );
        } catch (error) {
          this.logger.error(
            `Failed to trigger reminder ${reminder.id}:`,
            error,
          );
  ...
  async autoCompleteStaleReminders() {
    try {
      ...
      if (result.count > 0) {
        this.logger.log(
          `Auto-completed ${result.count} stale reminder(s) (triggered > ${AUTO_COMPLETE_GRACE_DAYS} days ago)`,
        );
      }
    } catch (error) {
