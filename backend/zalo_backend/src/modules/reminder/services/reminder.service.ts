...
export class ReminderService {
  ...
  async create(userId: string, dto: CreateReminderDto) {
    ...
    this.logger.log(
      `Reminder ${reminder.id} created for user ${userId} at ${remindAt.toISOString()}`,
    );
  ...
  async findOne(userId: string, reminderId: string) {
    ...
    if (reminder.userId !== userId)
      throw new ForbiddenException('Bạn không có quyền xem nhắc hẹn này.');
  ...
  async update(userId: string, reminderId: string, dto: UpdateReminderDto) {
    ...
    if (existing.userId !== userId)
      throw new ForbiddenException(
        'Bạn không có quyền chỉnh sửa nhắc hẹn này.',
      );
  ...
  private validateRemindAt(remindAt: Date) {
    ...
    if (delay < MIN_REMINDER_DELAY_MS) {
      throw new BadRequestException(
        'Thời gian nhắc hẹn phải ít nhất 1 phút trong tương lai.',
      );
    }
